 ...
 *
 * @brief CrossRef XML metadata export plugin
 */
...
class CrossRefExportPlugin extends DOIPubIdExportPlugin {
	...
	function getSubmissionFilter() {
		return 'submission=>crossref-xml';
	}
	...
	function updateDepositStatus($context, $object) {
		// assert(is_a($object, 'PublishedMonograph') or is_a($object, '[OTHER CLASS NEEDED]'));
		assert(is_a($object, 'PublishedMonograph'));
